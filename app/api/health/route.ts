        ...
        if (provider === 'auto') {
            try {
                ...
                if (!process.env.OPENROUTER_API_KEY) {
                    return NextResponse.json({ connected: false, provider: 'auto', message: 'Cloud API key missing' }, { status: 401 });
                }
                const res = await fetch('https://openrouter.ai/api/v1/auth/key', {
                    headers: { 'Authorization': `Bearer ${process.env.OPENROUTER_API_KEY}` },
                    signal: AbortSignal.timeout(5000)
                });
                return NextResponse.json({ connected: res.ok, provider: 'cloud', fallback: 'auto' });
            }
        }
