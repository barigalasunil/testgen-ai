...
export default function ApiTestingPage() {
    ...
    const handleSaveToJira = async () => {
        ...
        try {
            ...
            if (!credentials) {
                setJiraResult({ error: 'No Jira credentials. Go to TCGen-Buddy and open Jira Settings first.' });
                return;
            }
                            ...
                            {TEST_TYPES.map(t => (
                                <button key={t.key} onClick={() => setTestType(t.key)}
                                    className={cn("flex items-start gap-3 p-3 rounded-xl border text-left transition",
                                        testType === t.key ? "border-blue-500 bg-blue-50" : "border-slate-200 hover:border-slate-300 hover:bg-slate-50")}>
                                    ...
                                    <div>
                                        <p className={cn("text-sm font-semibold", testType === t.key ? "text-blue-700" : "text-slate-700")}>{t.label}</p>
