...
export function MainApp() {
    ...
    useEffect(() => {
        ...
        const interval = window.setInterval(() => {
            setActivityIndex((value) => (value + 1) % GENERATION_STEPS.length);
        }, 2200);
    ...
    const handleSend = async (overridePrompt?: string) => {
        ...
        try {
            ...
            const parsedResult =
                data &&
                !data.error &&
                isParsedTestCaseResult(data.result)
                    ? data.result
                    : null;
        ...
        setSessions(prev => prev.map(session =>
            session.id === activeId
                ? {
                    ...
                    automation: {
                        ...session.automation,
                        [suite]: { ...session.automation[suite], status: 'running', lastRunAt: startedAt },
                    },
                    updatedAt: startedAt,
                }
                : session
                ...
                session.id === activeId
                    ? {
                        ...
                        reports: payload.reportUrl
                            ? Array.from(new Set([...(session.reports || []), payload.reportUrl]))
                            : session.reports,
        ...
        <div className="flex h-screen bg-slate-50 text-slate-900 overflow-hidden font-sans">
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex flex-1 min-h-0 overflow-hidden">
                    ...
                    <aside className="hidden xl:flex h-full w-[340px] shrink-0 flex-col overflow-hidden border-l border-slate-200 bg-slate-50 p-4">
